...
from telegram.ext import ApplicationBuilder, CommandHandler, CallbackQueryHandler, ContextTypes, MessageHandler, ChatMemberHandler, filters
...
channel_memberships_collection = db["channel_memberships"]
...
def setup_database():
    ...
    channel_memberships_collection.create_index(
        [("user_id", 1), ("channel_key", 1)],
        unique=True
    )
...
# =========================
# AUTO CHANNEL HISTORY HELPERS
# =========================

def get_auto_channel_key(link):
    ...
    try:
        ...
        if not str(link or "").strip().startswith(valid_prefixes):
            return None
...
def user_completed_auto_channel(user_id, channel_key):
    ...
    return bool(
        users_collection.find_one(
            {
                "user_id": user_id,
                "completed_auto_channels": channel_key
            },
            {"_id": 1}
        )
    )
...
async def track_channel_membership(update, context):
    ...
    # Record a fresh join/rejoin event. This lets Auto tasks distinguish
    # users who joined after a task was published from users who were
    # already members before that task existed.
    if (
        new_status in active_statuses
        and old_status not in active_statuses
    ):
        channel_memberships_collection.update_one(
            {
...
async def tasks_menu(
    ...
    for task in tasks:
        ...
        # Only Auto Telegram Channel Join tasks use permanent
        # channel-level completion history. Other task types are untouched.
        if (
            task.get("task_mode", "auto") == "auto"
            and task.get("category") == "Telegram Channel Join"
        ):
            channel_key = task.get("auto_channel_key") or get_auto_channel_key(task.get("link"))

            if user_completed_auto_channel(user_id, channel_key):
                continue
...
async def task_details(
    ...
    if (
        ...
        if user_completed_auto_channel(user_id, channel_key):
            await query.answer(
                "❌ You already completed this channel task.",
                show_alert=True
            )
            return
...
async def complete_task(
    ...
    if category == "Telegram Channel Join":
        ...
        try:
            ...
            # A join event is required for new Auto tasks. If the bot has
            # no recorded join time, we cannot safely prove that the user
            # joined after this task was created, so do not award it.
            membership = channel_memberships_collection.find_one(
                {
                    "user_id": user_id,
                    "channel_key": channel_key
                }
            ...
            if (
                not joined_at
                or (task_created_at and joined_at <= task_created_at)
            ):
                await query.answer(
                    "❌ You must join the channel after this task was created.",
                    show_alert=True
                )
                return
    ...
    # Permanent channel-level history is used only for Auto Telegram
    # Channel Join tasks, so deleting/recreating a task cannot reset it.
    if category == "Telegram Channel Join":
        reward_filter["completed_auto_channels"] = {
            "$ne": channel_key
        }
        reward_update["$addToSet"]["completed_auto_channels"] = channel_key
            ...
            tasks_collection.insert_one(
                {
                    ...
                    "auto_channel_key": (
                        get_auto_channel_key(task["link"])
                        if task["task_mode"] == "auto"
                        and task["category"] == "Telegram Channel Join"
                        else None
                    ),
